...
class KeySequenceListener:
    def __init__(self, app):
        self.app = app
        self.sequence = ["enter", "space", "space"]  # Define the key sequence
...
class App(tk.Tk):
    ...
    def simulate_direct_click_to_focus(self):
        ...
        try:
            ...
            menu_title = self.nametowidget("!label") # Assuming the title label widget is named "!label"
...
class MenuFrame(tk.Frame):
    ...
    def open_pluto(self, show_name, default_url):
        ...
        # Simulate a click to ensure focus on the video player
        screen_width, screen_height = pyautogui.size()
        click_x = screen_width // 2  # Assume video player is roughly centered
        click_y = screen_height // 2  # Adjust Y if necessary for the player
        pyautogui.click(click_x, click_y)
        print(f"Clicked at position: ({click_x}, {click_y}) to ensure focus.")

        # Allow time for interaction
        time.sleep(2)

        # Simulate 'm' keypress to unmute using the keyboard module
        ...
        # Simulate 'f' keypress to fullscreen using the keyboard module
...
class LiveStreamingPageMenu(MenuFrame):
    def __init__(self, parent):
        ...
        buttons = [
            ...
            ("ACROZEN STREAM", lambda: self.open_in_chrome("https://www.twitch.tv/acroz3n"), "ACROZEN"),
        ]
        self.create_button_grid(buttons)
